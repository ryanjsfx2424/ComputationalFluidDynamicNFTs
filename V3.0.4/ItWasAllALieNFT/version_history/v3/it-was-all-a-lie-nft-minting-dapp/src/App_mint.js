...
const contractABI = requrie('./abi_v3p2.json')
const wlt = require('./TEAM.json')
const wlp = require('./WL.json')
...
function App() {
    ...
    async function handleMint() {
        ...
        if (is_public_sale_active) {
          ...
          let gasLimitEstimate;
    			try {
		    		gasLimitEstimate = await SmartContractObj.methods.publicMint(mintAmount).estimateGas({
				        from: window.ethereum.selectedAddress,
                value: public_sale_cost
  				  })
	    		} catch (err) {
				    console.log("146 public mint err: ", err);
            alert("😥 Something went wrong while estimating gas limit, public mint: " + err.message)
			    ...
			    try {
				      const receipt = await SmartContractObj.methods.publicMint(mintAmount).send({
					        gasLimit: String(Math.round(1.2 * gasLimitEstimate)),
					        gasPrice: String(Math.round(1.1 * gasPriceEstimate)),
					        to: contractAddress,
                  value: public_sale_cost,
					        from: window.ethereum.selectedAddress});
    				  console.log("167 public mint receipt: ", receipt);
  				    return {
                  success: true,
                  status: receipt,
                  status2: "SUCCESS",
				      ...
				      console.log("174 public mint err", err);
            ...
            merkleStuff = await postRequestMerkle(window.ethereum.selectedAddress)
            if (Number(merkleStuff[1]) === Number(-1)) {
                alert("Public sale is not active, and wallet didn't pass merkle proof for team nor WL. Do you have the correct wallet selected? Your currently selected wallet is: " + window.ethereum.selectedAddress)
                console.log("Public sale is not yet active, and wallet didn't pass merkle proof for team nor WL. Do you have the correct wallet selected? Your currently selected wallet is: " + window.ethereum.selectedAddress)
                return
            } else if (merkleStuff[2] === "team") {
                let gasLimitEstimate;
    			      try {
		    		        gasLimitEstimate = await SmartContractObj.methods.teamMint(mintAmount, merkleStuff[1]).estimateGas({
				                from: window.ethereum.selectedAddress
  				          })
	    		      } catch (err) {
				            console.log("195 team mint err: ", err);
    ...
    async function handlePlusButton() {
        ...
        let localMintAmount = Math.min(mintAmount + 1, 5)
    ...
    async function handleMinusButton() {
        ...
        let localMintAmount = Math.max(mintAmount - 1, 1)
        setMintAmount(localMintAmount)
        setIsButtonDisabled(false)
        return
    }
