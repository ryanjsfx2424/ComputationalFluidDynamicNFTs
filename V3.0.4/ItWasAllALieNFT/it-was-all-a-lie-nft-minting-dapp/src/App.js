    ...
    async function handleMint() {
        ...
        if (is_public_sale_active) {
            ...
            if (Number(merkleType) === Number(-1)) {
                ...
                setIsButtonDisabled(false)
                return
            } else if (merkleType === "team") {
                let gasLimitEstimate;
    			try {
		    		gasLimitEstimate = await SmartContractObj.methods.teamMint(mintAmount, merkleProof).estimateGas({
				        from: window.ethereum.selectedAddress
  				    })
	    		} catch (err) {
                ...
                let gasLimitEstimate;
    			try {
		    		gasLimitEstimate = await SmartContractObj.methods.wlMint(mintAmount, merkleProof).estimateGas({
				        from: window.ethereum.selectedAddress,
                        value: totalCostWei
  				    })
	    		} catch (err) {
