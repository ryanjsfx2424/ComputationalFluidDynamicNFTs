...
for ii,fname in enumerate(fs):
  ...
  for jj in range(len(TYPES)):
    arr = np.array(stou[TYPES[jj]]) / float(stou["followers"]) * 2e4
...
plt.ylabel("Counts / 20k Followers",fontsize=LAB)
...
plt.tight_layout()
plt.savefig("lrrtqt_per20kFollowers_llamas_roos_seals_gac.png")
plt.close()
## end plot.py
