...
for username in usernames:
  fname = DATA_DIR + "/" + username.lower() + EXT
...
for ii in range(4):
    ...
    ax.set_xticklabels(months[-1:], **csfont)
    ax.tick_params(axis="x", which="major", labelsize=XTI)
    ax.tick_params(axis="x", which="minor", labelsize=XTI)
    ax.tick_params(axis="y", which="major", labelsize=YTI)
    ax.tick_params(axis="y", which="minor", labelsize=XTI)
# end for ii
...
plt.tight_layout()
plt.savefig("roos_vs_llamas_vs_seals_May.png")
plt.close()
