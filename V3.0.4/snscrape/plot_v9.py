...
for ii,fname in enumerate(fs):
  ...
  for jj in range(len(TYPES)):
    ...
    inds = np.where(arr > REACTIONS)
    num_tweets = len(np.array(stou["dates"])[inds])
    arr  = arr[inds] / float(stou["followers"]) * 2e4 / float(num_tweets) * 100
    yvals.append(np.median(arr))
    xvals.append(cnt+jj*OFFSET)
    yerrs.append(np.percentile(arr, [5,95]))
    #colors.append(color)
    #markers.append(marker)
    #yvals[-1] = max(yvals[-1],1)
    ymax = max(yerrs[-1][1], 1)
    plt.scatter(xvals[-1], yvals[-1], c=colors[jj], marker=markers[jj])
    plt.vlines(xvals[-1], ymin=yerrs[-1][0], ymax=ymax, color=colors[jj])
  # end for
# end for
plt.title("Twitter Engagement (Tweets > " + str(REACTIONS) + " reactions)",fontsize=TIT)
plt.ylabel("Counts / 20k Followers / 100 Tweets",fontsize=LAB)
plt.yticks(fontsize=XTI)
plt.yscale("log")
plt.xticks([1.2,3], usernames[::2], fontsize=XTI)
plt.xlim(0.85,4.5)
...
plt.tight_layout()
plt.savefig("lrrtqt_per20kFollowersPer100TweetsGtr" + str(REACTIONS) + "Reactions_llamas_roos_seals_gac.png")
plt.close()
## end plot.py
