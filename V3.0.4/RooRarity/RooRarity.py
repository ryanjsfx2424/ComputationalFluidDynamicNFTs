...
## this is the interactions discord library with a small tweak
## (just adds get.py); you can find it on my github ryanjsfx2424
    ...
    async def roorarity(ctx: interactions.CommandContext,
      ...
      if   rank != None and name != None:
        ...
        if rank < 1 or rank > 5501:
          await ctx.send("error! rank must be >=1 and <= 5500",
                       ephemeral=True)
          return
        # end if
        ...
        description = "\u200b" # == empty; discord doesn't like empty string
        ...
        for ii in range(len(self.rarities)):
          if   name1 == self.rarities[ii] or \
            ...
            home = os.getcwd()
            os.chdir("/Users/ryanjsfx/Documents/RooPunks/IMAGES/root/")
            #embed.set_image(url=self.URL)#image_url)
