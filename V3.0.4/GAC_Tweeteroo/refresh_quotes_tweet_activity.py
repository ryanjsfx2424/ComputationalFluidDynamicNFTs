## refresh_fetch_quotes_tweet_activity.py
import os
import ast
import time
...
def refresh_fetch_quotes_activity(tweet_id):
    """tweet_id should be a string"""
    print("[tweet_id]: ", [tweet_id])
    print("begin refresh_fetch_quotes_activity")
    ...
    if token == "None" and len(lines) > len(last_lines):
        lines = lines[:-6]
        last_lines = lines[-6:]
        token = last_lines[0]
        token = token.split("next_token: ")[1]
    ...
    print("SUCCESS refresh_fetch_quotes_activity")
    return True
# end refresh_fetch_quotes_activity
...
if __name__ == "__main__":
    ...
    for tweet_id in tweet_ids:
        ...
        result = refresh_fetch_quotes_activity(tweet_id)
        if result:
          print("sleeping a minute between quotes fetches")
          time.sleep(LONG_SLEEP)
        # end if
    # end for
    print("SUCCESS refresh_fetch_quotes_tweet_activity")
# end if __name__ == "__main__"
## end refresh_fetch_quotes_tweet_activity.py