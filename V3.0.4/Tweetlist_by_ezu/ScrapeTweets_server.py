...
class ScrapeTweets(object):
  def __init__(self):
    ...
    '''
    self.refresh_likes_num_hours = 24
    self.stop_scrape = False
    self.dev_mode = False
    self.dev_mode_no_cmds = False
    self.dev_mode_no_stream = False
    self.fetch_likes_sleep_interval = 3600.0
    self.approved_users = [855616810525917215]
    self.do_fetch_likes = False
    self.pages = {}
    ...
    self.BOT_COMMANDS_CIDS = [932056137518444594, # toTheMoons 
                              ...
                              996924823903162408] # roo tech, helle
...
#                             "1477912158730170370":"troopsales"}
    ...
    #self.keyword_help = self.keyword_query + " OR rootyroo OR RootyRoo OR rootroopnft OR troopsales OR 🌳🦘)"
    #self.keyword_query += " OR rootyroo OR RootyRoo OR rootroopnft OR troopsales OR 🌳🦘)"
    #self.keyword_query += " OR rootyroo OR RootyRoo OR rootroopnft OR troopsales OR 🌳🦘)"
    #self.keyword_query += " OR rootyroo OR RootyRoo OR rootroopnft OR troopsales OR \uD83C\uDF33\uD83E\uDD98)"
    self.keyword_help = self.keyword_query
    ...
    msg2 = "Hi! Below are the keywords I use to scrape for tweets.\n\n"
    msg2 += tab + "Note that I'm insensitive to case and puncutation and spaces are wild.\n\n"
    msg2 += tab + "For example, the 'rootyroo' keyword below tracks 'rootyroo!' AND 'rooTyrOo' AND '#rootyroo', AND EVEN '$rootyroo' etc.\n\n"
    msg2 += tab + "The 'rooty roo' rule covers 'rooty roo' AND 'rooty fkn roo' AND 'rooty fkn slap wab like a mofo roo' etc.\n\n"
    msg2 += tab + "Hope that helps clarify! You can always ping me 'RyanRoo...' with additional questions :)\n\n"
    msg2 += tab + "And with that said, here are the keywords!\n\n"
    for keyword in self.keyword_help.split("OR"):
    ...
    self.help     = help_msg_base + \
                    ...
                    "**__!rttverify <url>, <twitter username>__**\n" + tab  + \
                    "Verify if we've processed your interaction\n\n" + \
                    "**__!rttstats <twitter username>__**\n" + tab + \
    ...
    for ii in range(len(self.user_dict["discordId_to_username"])):
      ...
      tun = self.user_dict["discordId_to_username"][did]
      uid = self.user_dict["username_to_userId"][tun]
      if tun not in self.linked_usernames:
        self.linked_usernames.append(str(tun))
        self.linked_userIds.append(str(uid))
        self.linked_discordIds.append(str(did))
      # end if
    # end for ii
    #print(self.linked_userIds)
    with open("foo4242.txt", "w") as fid:
      fid.write(str(self.linked_userIds))
    # end with
  ...
  async def save_url_to_file(self, url, fname):
    ...
    cnt = 0
    while flag:
      cmd = self.curl_base + url + self.curl_header + self.auth + \
                "' --connect-timeout 30 --max-time 30 >> " + fname
      print("cmd: ", cmd.encode("utf-16", "surrogatepass").decode("utf-16"))
      result = os.system(cmd.encode("utf-16", "surrogatepass").decode("utf-16"))
  ...
  async def fetch_data(self, tweet_url, dtype):
    ...
    while loop and num_loops < self.max_loops:
      ...
      try:
        ...
        print("726 err: ", err)
        print("727 err args: ", err.args[:])
        loop = False
        break
      # end try/except
       
      if type(line) == type( (1,1) ):
        line = list(line)
      # end if
      if type(line) != type([]):
        line = ["foo",line]
      ...
      line = line[1:]
      for vals in line:
        if "meta" not in vals.keys():
          continue

        if "next_token" in vals["meta"]:
          token = vals["meta"]["next_token"]
          url = url_og + "&pagination_token=" + token
        else:
          print("next token not in line 727")
          loop = False
        # end if/else
      # end for
    # end while
  ...
  async def process_all_tweets_by_user(self, user_id, update = False):
    ...
    for ii in range(len(line)):
      ...
      for jj in range(len(line[ii]["data"])):
        ...
        num_hours = self.refresh_likes_num_hours
        tnow = datetime.datetime.now() - datetime.timedelta(hours=num_hours)
        ...
        if tweet_id in processed_tweets and creation_time_s < refresh_time:
          print("tweet_id in processed_tweets and older than " + str(num_hours) + " days, so skipping")
          continue
        # end if
  ...
  async def fetch_rank(self, username, start_time="2020-05-04T23:59:59.000Z", 
    ...
    for user in list(self.activity_by_user.keys()):
      ...
      keyword_cnts.append(keywords_cnt)
      ...
      # end for
      if False:#self.user_dict["userId_to_username"][user] == "morganstoneee":
        print("hi")
        print()
    ...
    keyword_cnts = np.array(keyword_cnts)
    ...
    #  val = num_QuoteTweets
      ...
      inds = np.argsort(keyword_cnts)[::-1]
      val = keyword_cnts
    # end if/elifs

    username = username.lower().replace(" ","").replace("\n","")
    #print("1698 un fr passed in: ", username)
    #print("1715  uns[inds][:85]: ", usernames[inds][:85])
    #print("1716 len uns: ", len(usernames))
    #print("username == thelunalabs: ", username == "thelunalabs")
    try:
  ...
  async def fetch_user_leaderboard(self, start_time="2020-05-04T23:59:59.000Z", 
    ...
    custom = ""
    print("1744 method: ", method)
    if "Custom_" in method:
      custom += method
      method = method.split("Custom_")[1]
      print("1748 method: ", method)
    # end if
    ...
    for user in list(self.activity_by_user.keys()):
      ...
      if custom != "":
        num_dict["num_Custom"].append(keywords_cnt)
      # end if
      keyword_replies.append(replies_cnt)
      keyword_retweets.append(retweets_cnt)

      #if "kevwants_crypto" in self.activity_by_user[user]["usernames"]:
      #  print("kevwants_crypto exists!")
      #  print("1843 uns: ", self.activity_by_user[user]["usernames"])
      ...
      if False:#self.user_dict["userId_to_username"][user] == "morganstoneee":
        ...
        print("num_Replies: ", num_dict["num_Replies"][-1])
        print("num_QuoteTweets: ", num_dict["num_QuoteTweets"][-1])
      if False:#self.user_dict["userId_to_username"][user].lower() == "gio3272":
        print("hi")
        print()
        print("un: ", self.user_dict["userId_to_username"][user])
        print("keyword_replies: ", keyword_replies[-1])
    ...
    num_Replies = np.array(num_dict["num_Replies"])
    if custom != "":
      num_Custom = np.array(num_dict["num_Custom"])
    # end if
    ...
    inds = np.argsort(points)[::-1]
    if start_time == "2020-05-04T23:59:59.000Z" and end_time == "4022-05-04T23:59:59.000Z" and method == "Points":
  ...
  async def continuously_scrape(self):
    ...
    if not self.dev_mode_no_stream:
    ...
    while True:
      ...
      if time.time() - refresh_time > self.fetch_likes_sleep_interval or \
         self.do_fetch_likes:
        refresh_time = time.time()
        if self.do_fetch_likes:
          self.do_fetch_likes = False
        # end if
  ...
  def discord_bot(self):
    ...
    async def cmd(ctx: interactions.CommandContext, sub_command: str,
      ...
      if self.dev_mode_no_cmds and ctx.author.id not in self.approved_users:
        await ctx.send("Commands disabled, we're currently in dev mode")
        return
      # end if
      tab = self.tab
      if   sub_command in ["help", "halp", "hlp"]:
        ...
        if "https://twitter.com/" not in tweet_url:
          ...
          try:
            ...
            print("3040 err: ", err)
            print("3041 err args: ", err.args[:])
            msg2 = ">>> an error occurred. Did you #link-twitter?"
            await ctx.send(msg2, ephemeral=True)
            status = False
            return
          # end try/except
    ...
    async def on_message(message):
      ...
      if message.channel.id == 982340728980140122:
        ...
        self.linked_usernames  = [str(tun )] + self.linked_usernames
        self.linked_userIds    = [str(twid)] + self.linked_userIds
        self.linked_discordIds = [str(did )] + self.linked_discordIds
      ...
      if message.author.id in self.approved_users:
        if   message.content == "!rtt doFetchLikes":
          self.do_fetch_likes = True
          await channel.send("self.do_fetch_likes set to True")
          return
        elif "!rtt updateUserInfo" in message.content:
          try:
            tun,did,twid = message.conent.split(":")[1].split(", ")
            ...
            print("3616 err: ", err)
            print("3617 err.args: ", err.args[:])
            print("err in updateUserInfo parsing mssage.content: ", message.content)
            await channel.send("error parsing updateUserInfo command. Do better :P")
          # end try/except
          return
        elif message.content == "!rtt fetchLikesNow":
          await channel.send("going to fetch likes now")
          for cs_user_id in list(self.special_tweeters.keys()):
            await self.process_all_tweets_by_user(cs_user_id, update=True)
          # end for
          await channel.send("done fetching likes")
          return
        elif message.content == "!rtt loadABU":
          if not self.dev_mode:
            await channel.send("not going to load abu since not in dev mode")
            return
          # end if
          await channel.send("going to load abu")
          self.activity_by_user = self.safe_load_abu()
          await channel.send("finished loading abu")
          return
        elif "!rtt updateLikeRefreshTime" in message.content:
          msg = message.content.split(":")[1].replace(" ","").replace("\n","")
          msg = int(float(msg))
          self.fetch_likes_sleep_interval = msg
          await channel.send("set fetch likes sleep interval to: " + str(msg))
          return
        elif "!rtt devMode=True"  == message.content:
          self.dev_mode = True
          await channel.send("set dev_mode True")
          return
        elif "!rtt devMode=False" == message.content:
          self.dev_mode = False
          await channel.send("set dev_mode False")
          return
        elif "!rtt devModeNoCmds=True" == message.content:
          self.dev_mode_no_cmds = True
          await channel.send("set dev_mode no cmds True")
          return
        elif "!rtt devModeNoCmds=False" == message.content:
          self.dev_mode_no_cmds = False
          await channel.send("set dev_mode no cmds False")
          return
        elif "!rtt devModeNoStream=False" == message.content:
          self.dev_mode_no_stream = False
          await channel.send("set dev_mode no stream False")
          return
        elif "!rtt devModeNoStream=True" == message.content:
          self.dev_mode_no_stream = True
          await channel.send("set dev_mode no stream True")
          return
        elif "!rtt startStream" == message.content:
          os.system("python3 stream.py &")
          await channel.send("started streaming")
          return
        elif "!rtt startScrape" == message.content:
          await channel.send("going to continuously scrape")
          await self.continuously_scrape()
          return
        elif "!rtt stopScrape" == message.content:
          self.stop_scrape = True
          await channel.send("set stop scrape True")
          return
        elif "!rtt setRefreshLikesNumHours" in message.content:
          num_hours = float(message.content.split("=")[1].replace(" ","").replace("\n",""))
          self.refresh_likes_num_hours = num_hours
          await channel.send("set refresh_likes_num_hours to " + str(num_hours))
          return
        # end if/elifs
        elif "!rtt addBotChannel" in message.content:
          try:
            chid = int(float(message.content.split("=")[1]))
            self.BOT_COMMANDS_CIDS.append(chid)
          except:
            await channel.send("error parsing channel to add for tweeteroo commands, recvd: " + message.content)
          # end try/except
        # end elifs
        elif "!rtt addGuilds" in message.content:
          try:
            guid = int(float(message.content.split("=")[1]))
            self.GUILDS.append(guid)
          except:
            await channel.send("error parsing channel to add for tweeteroo commands, recvd: " + message.content)
          # end try/except
        # end elifs
      # end if
      if self.dev_mode_no_cmds and message.author.id not in self.approved_users:
        if "!rtt" in message.content:
          await channel.send("commands disabled, we're currently in dev mode")
        return
      # end if
      ...
      if msg.startswith("rtt") or msg.startswith("!rtt") or \
        ...
        if "help" in msg[:max_len]:
            ...
            try:
              ...
              try:
                ...
                try:
                  ...
                  msg2  = ">>> sorry, I couldn't parse that. I'm loooking for something like\n"
                  msg2 += 2*tab + "*!rttstats TheLunaLabs*"
                  await channel.send(msg2)
                  return
                # end try/except
              # end try/except
          ...
          if "username" in msg:
            ...
            try:
              ...
              try:
                ...
                msg2  = ">>> sorry, I couldn't parse that. I'm loooking for something like\n"
                msg2 += 2*tab + "*!rttverify https://twitter.com/RooTroopNFT/status/1499858580568109058, TheLunaLabs*"
                await channel.send(msg2)
                return
              # end try/except
            # end try/except
            ...
            if "," in msg:
              url,username = msg.split(",")
              url = url.replace("!","").replace("rttverify","").replace(" ","")
              if "?" in url:
                url = url.split("?")[0]
              # end if
              tweet_url = url
              ...
              try:
                ...
                try:
                  url = msg.replace("!","").replace("rttverify","").replace(" ","")
                  if "?" in url:
                    url = url.split("?")[0]
                  # end if
                  tweet_url = url
          ...
          if "start:" in msg and "end:" in msg:
            ...
            end_time   = "2023-01-01T00:00:00.000Z"
            time_str = "2022"
          elif "last month" in msg or "june" in msg:
            print("last month or june in msg")
            start_time = "2022-06-01T00:00:00.000Z"
            end_time   = "2022-07-01T00:00:00.000Z"
            time_str = "June 2022"
          elif "month" in msg or "july" in msg:
