...
class EzuTweeteroo(object):
    ...
    def load_keywords(self):
        ...
        for tweet in tweets:
            ...
            dates_s.append(self.get_tweet_time_s(dates[-1]))

            tweet_ids.append(int(tweet.split("/status/")[1].split("'")[0]))

            tuids.append(int(tweet.split("user=User(")[1].split("id=")[1].split(",")[0]))
    ...
    def load_engagement(self):
        ...
        for etype in ["likes", "retweets", "replies", "quotes"]:
            ...
            self.engagement[etype] = {
                "tweet_ids": np.array(tweet_ids).astype(np.int),
                "tuids": np.array(tuids).astype(np.int),
                "usernames": usernames
            }
        # end for etypes
    # end load_engagement
    ...
    def load_latest_tweet_time_s(self):
        ...
        if self.stream_data["dates_s"] != []:
            self.latest_tweet_time_s = np.max(np.array(self.stream_data["dates_s"]))
        # end if
        self.latest_tweet_time_s = max(np.max(self.keywords_data["dates_s"]),
    ...
    def get_user_stats(self, username):
        ...
        indsST = np.where(np.array(self.stream_data["usernames"]) == username)

        indsSK = np.where(np.array(self.stream_data["etypes"])[indsST] == self.FLAG_KEY)
        indsSR = np.where(np.array(self.stream_data["etypes"])[indsST] == self.FLAG_RTS)
        indsSQ = np.where(np.array(self.stream_data["etypes"])[indsST] == self.FLAG_QTS)
        ...
        lenKE += len(np.array(self.stream_data["usernames"])[indsST][indsSK])
        lenRT += len(np.array(self.stream_data["usernames"])[indsST][indsSR])
        lenQT += len(np.array(self.stream_data["usernames"])[indsST][indsSQ])
    ...
    def verify_processed_tweet(self, tweet_url, username):
        ...
        if "ezu_xyz" in tweet_url:
            ...
            for etype in ["likes","retweets"]:
                ...
                # end if
                if etype == "retweets":
                    inds = np.where(np.array(self.stream_data["etypes"]) == self.FLAG_RTS)
                    inds = np.where(np.array(self.stream_data["tweet_ids"])[inds] == tweet_id)
                    if len(inds) != 0:
                        print("705 inds: ", inds)
                        if username in np.array(self.stream_data["usernames"])[inds]:
                            return [self.VERIFY_SUCCESS, True]
                        # end if
                    # end if
                # end if
            ...
            for etype in ["quotes", "replies"]:
                ...
                # end if
            # end for
            inds = np.where(np.array(self.stream_data["etypes"]) == self.FLAG_QTS)
            inds = np.where(np.array(self.stream_data["tweet_ids"])[inds] == tweet_id)
            if len(inds) != 0:
                print("732 inds: ", inds)
                if username in np.array(self.stream_data["usernames"])[inds]:
                ...
                # end if
            # end if
            inds = np.where(np.array(self.stream_data["etypes"]) == self.FLAG_KEY)
            inds = np.where(np.array(self.stream_data["tweet_ids"])[inds] == tweet_id)
            if len(inds) != 0:
                print("740 inds: ", inds)
                if username in np.array(self.stream_data["usernames"])[inds]:
    ...
    async def get_rankings(self, method = DEFAULT_METHOD,
        ...
        if method in ["Likes", "Retweets"]:
            ...
            inds = np.where(np.array(self.stream_data["etypes"]) == etype)
            tuids = np.unqiue(
                        np.concatenate([
                            self.engagement[etype]["tuids"],
                            np.array(self.stream_data["tuids"])[inds]
                        ])
            )
            ...
            for ii,tuid in enumerate(tuids):
                ...
                indsST = np.where(np.array(self.stream_data["tuids"])[inds] == tuid)

                lenEE  = len(self.engagement[etype]["usernames"][indsEE])
                lenEE += len(      np.array(self.stream_data["usernames"])[inds][indsST])

                ees[ii] = lenEE
                if len(self.engagement[etype]["usernames"][indsEE]) != 0:
                    usernames.append(self.engagement[etype]["usernames"][indsEE][0])
                elif len(np.array(self.stream_data["usernames"])[inds][indsST]) != 0:
                    usernames.append(np.array(self.stream_data["usernames"])[inds][indsST])
                # end if/elifs
            # end for
            ...
            tuids =  np.unique(
                    np.concatenate([
                        ...
                        np.array(self.stream_data["tuids"])
                    ])
                )
            ...
            for ii,tuid in enumerate(tuids):
                ...
                indsST = np.where(np.array(self.stream_data["tuids"]) == tuid)
                ...
                elif len(np.array(self.stream_data["usernames"])[indsST][indsSK]) != 0:
                    usernames.append(np.array(self.stream_data["usernames"])[indsST][indsSK])
                elif len(np.array(self.stream_data["usernames"])[indsST][indsSQ]) != 0:
                    usernames.append(np.array(self.stream_data["usernames"])[indsST][indsSQ])
                # end if/elifs
            # end for
            ...
            for ii,tuid in enumerate(tuids):
                ...
                if   len(self.keywords_data["usernames"][indsKE]) != 0:
                    ...
                    usernames.append(np.array(self.stream_data["usernames"])[indsST][indsSK])
                elif len(np.array(self.stream_data["usernames"])[indsST][indsSR]) != 0:
                    usernames.append(np.array(self.stream_data["usernames"])[indsST][indsSR])
                elif len(np.array(self.stream_data["usernames"])[indsST][indsSQ]) != 0:
                    usernames.append(np.array(self.stream_data["usernames"])[indsST][indsSQ])
                # end if/elifs
            # end for
            ...
            for fn in fs:
                ...
                with open(fn, "r") as fid:
                    for line in fid:
                        ...
                        date_s   = self.get_tweet_time_s(date)
                        ...
                        for user in line["includes"]["users"]:
                            ...
                            # end if
                        # end for
                        self.stream_data["tweet_ids"  ].append(tweet_id)
                        self.stream_data[    "tuids"  ].append(tuid)
                        self.stream_data["usernames"  ].append(username)
                        self.stream_data[    "dates"  ].append(date)
                        self.stream_data[    "dates_s"].append(date_s)
                        self.stream_data[   "etypes"  ].append(flag)
                        self.stream_data[    "texts"  ].append(text)

                        self.safe_save(self.fname_stream, self.stream_data)
                    # end for line in fid
                # end with open
            # end for fs
        # end while True
