...
class EzuTweeteroo(object):
    ...
    def load_latest_tweet_time_s(self):
        self.latest_tweet_time_s = max(np.max(self.keywords_data["dates_s"]),
                                       np.max(  self.stream_data["dates_s"]))
    ...
    async def get_rankings(self, method = DEFAULT_METHOD,
        ...
        if method in ["Likes", "Retweets"]:
            etype = method.lower()
            tuids, indsU, count = np.unique(self.engagement[etype]["tuids"], return_counts=True, return_index=True)
            inds = np.argsort(count)[::-1]
            vals = count[inds]
            usernames = self.engagement[etype]["usernames"][indsU][inds]
