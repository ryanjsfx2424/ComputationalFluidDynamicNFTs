//========
// rt2 modified from rt.cpp by ToTheMoonsNFT (aka TheLunaLabs)
//   Just going to set the perturbation wavelength (for iprob=1) to be 1/2
// so there should be two "fingers". (Actually used 'period' parameter to
// give myself more flexibility ;) )
// Also note that the below "Note the grav..." is a lie :)
//
//========================================================================================
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real gm1 = gamma - 1.0;
  Real period = pin->GetReal("problem","period");

  Real kx = 2.0*period*(PI)/(pmy_mesh->mesh_size.x1max - pmy_mesh->mesh_size.x1min);
  Real ky = 2.0*period*(PI)/(pmy_mesh->mesh_size.x2max - pmy_mesh->mesh_size.x2min);
  Real kz = 2.0*period*(PI)/(pmy_mesh->mesh_size.x3max - pmy_mesh->mesh_size.x3min);
