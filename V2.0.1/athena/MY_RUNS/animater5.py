...
for jj,plotdir in enumerate(plotdirs_saved):
  ...
  os.system("mkdir -p " + HOME + "last_frames")
  fs = list(np.sort(glob.glob("*.png")))
  os.system("rm " + str(jj) + ".png")
  os.system("cp " + fs[int(len(fs)/2)] + " " + HOME + "last_frames/" + str(jj).zfill(5) + ".png")
# end for
os.chdir(HOME + "last_frames")

input_name  = "%05d.png"
output_name = MOVIEDIR + "hidden.mp4"
