...
class ScrapeTweets(object):
  ...
  def process_activity(self, tweet_id):
    ...
    activity_new = {}
    key_ext = "_usernames_csv"
    for key in self.activity.keys():
      activity_new[key] = self.activity[key]
      ...
      for item in items:
        activity_new[new_key] += item["username"] + ","
      # end for item
      activity_new[new_key] = activity_new[new_key][:-1]
    # end for keys
    self.activity = activity_new

    print(self.activity["replies_includes_usernames_csv"])
    sys.exit()
    ...
    with open(self.data_dir + "/RooTroopActivity.csv", "a") as fid:
      ...
      csv_keys = ["likes"        + key_ext,
                  "retweets"     + key_ext,
                  "quote_tweets_includes" + key_ext,
                  "replies_includes"      + key_ext
                 ]
      print(self.activity.keys())
      for key in csv_keys:
        name = key.split("_")[0]
        fid.write(name + "," + self.activity[key] + "\n")
      # end for
      fid.write("\n\n")
    # end with open
...
tweet_ids = ["1515192646578290688", 
             "1515139514880061445", 
             "1505620615817953282"]
tweet_ids = ["1505620615817953282"]

## this one dies :(
tweet_ids = ["1515347532678901766"]
