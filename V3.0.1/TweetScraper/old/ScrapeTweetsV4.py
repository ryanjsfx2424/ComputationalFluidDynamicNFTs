...
class ScrapeTweets(object):
  ...
  def process_activity(self, tweet_id):
    ...
    key_ext_use = "_usernames_csv"
    for key in self.activity.keys():
      if "includes" in key:
        continue
      # end if
      activity_new_ids[key] = self.activity[key]
      activity_new_use[key] = self.activity[key]

      new_key_ids = key + key_ext_ids
      new_key_use = key + key_ext_use
      activity_new_ids[new_key_ids] = ""
      activity_new_use[new_key_use] = ""

      items_ids = self.activity[key]
      items_use = self.activity[key]

      if "includes" in key:
        items_use = self.activity[key]["users"]
      # end if

      item_key_use = "username"
      item_key_ids = "id"
      if key in ["quote_tweets", "replies"]:
        item_key_ids = "author_id"
      # end if

      for item in items:
        activity_new_ids[new_key_ids] += item[item_key_ids] + ","
        activity_new_use[new_key_use] += item[item_key_use] + ","
      # end for item
      activity_new_ids[new_key_ids] = activity_new_ids[new_key_ids][:-1]
      activity_new_use[new_key_use] = activity_new_use[new_key_use][:-1]
    # end for keys
    self.activity = activity_new_ids

    ## next, get the usernames corresponding to the ids, which
    ## is more human readable :)

    with open(self.data_dir + "/RooTroopActivity.csv", "a") as fid:
      fid.write("Tweet ID," + tweet_id + "\n")

      csv_keys = ["likes"        + key_ext,
                  "retweets"     + key_ext,
                  "quote_tweets" + key_ext,
                  "replies"      + key_ext
                 ]
