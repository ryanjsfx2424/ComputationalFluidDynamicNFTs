...
class ScrapeTweets(object):
  ...
  def fetch_activity(self, tweet_url, creation_time, update=False):
    ...
    for dtype in self.dtypes:
      ...
      # end with open
      key = '}]}{'
      line = line.split(key)[1] # skip public metrics

      sections = line.split(self.result_text)[:-1]
      for line in sections:
        if include_text in line:
          line2,line3 = line.split(include_text)
          ...
          activity = self.build_activity_from_pairs(pairs, activity, line2)
          self.update_user_dict(line3)
        else: # no include_text so either likes or retweets
          pairs = {'"id":"' : '"ids":['}
          activity = self.build_activity_from_pairs(pairs, activity, line)
          self.update_user_dict(line)
        # end if
