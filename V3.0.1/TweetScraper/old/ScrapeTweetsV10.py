  ...
  def fetch_data(self, tweet_url, dtype):
    ...
    or replied, and saves json response to a file for later processing.
      inputs: 
        tweet_url (type == str)
        dtype (type == str) 
          valid values == 'likes', 'retweets', 'quote_tweets', 'replies',
      outputs: none
    ...
    url_og = self.twitter_api_base + tweet_id
    if dtype == "likes":
      url_og += "/liking_users?user.fields=username&max_results=100"
      fname = self.data_dir + "/users_liked_tweet_" + tweet_id + ".txt"
    elif dtype == "retweets":
      url_og += "/retweeted_by?user.fields=username&max_results=100"
